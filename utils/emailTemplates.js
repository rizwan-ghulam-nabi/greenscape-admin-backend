...
// new version 24/09/26
// backend/utils/emailTemplates.js - PRODUCTION VERSION

const APP_URL = process.env.FRONTEND_URL || 'https://greenscape.vercel.app';
const SUPPORT_EMAIL = process.env.SUPPORT_EMAIL || 'sheikhrizwanghulamnabi555@gmail.com';
const YEAR = new Date().getFullYear();

// Safe number formatting (handles strings, undefined, NaN)
const fmt = (n) => {
  const num = Number(n);
  return Number.isFinite(num) ? num.toFixed(2) : '0.00';
};

// Hex (#RRGGBB) -> rgba() for better email client support than #RRGGBBAA
...
export const getPaymentStatusEmailTemplate = (order, paymentStatus) => {
  ...
  const getStatusMessage = (status) => {
    const messages = {
      ...
      refunded:
        'Your refund has been processed successfully. It will appear in your account within 5-7 business days.',
      pending:
        "Your payment is currently pending. We'll notify you once it's confirmed.",
    };
    return messages[status] || 'Your payment status has been updated.';
  };

  const color = getStatusColor(paymentStatus);
  const softBg = hexToRgba(color, 0.08);
  const softBorder = hexToRgba(color, 0.18);
  const softShadow = hexToRgba(color, 0.25);

  return {
    subject: `${getStatusIcon(paymentStatus)} Payment ${
    ...
    html: `
      ...
      <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background: #f0f4f8; margin: 0; padding: 40px 20px; -webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale;">
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 580px; margin: 0 auto; background: #ffffff; border-radius: 20px; overflow: hidden; box-shadow: 0 20px 60px rgba(0,0,0,0.08);">
          <tr>
            <td style="background: linear-gradient(135deg, #1a472a 0%, #2B7A4B 50%, #3a9d62 100%); padding: 40px 30px 35px; text-align: center;">
              <table width="100%" cellpadding="0" cellspacing="0">
          ...
          <tr>
            <td style="padding: 40px 30px;">
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td style="text-align: center; padding-bottom: 24px;">
                    <span style="display: inline-block; background: ${softBg}; color: ${color}; padding: 8px 20px; border-radius: 50px; font-size: 13px; font-weight: 600; border: 1px solid ${softBorder};">
                      Order #${order.orderNumber}
                    </span>
                  </td>
                </tr>
                              ...
                              <td style="padding-bottom: 12px;">
                                <table width="100%" cellpadding="0" cellspacing="0">
                                  <tr>
                                    ...
                                    <td style="width: 50%; text-align: right;">
                                      <p style="color: #94a3b8; font-size: 11px; margin: 0 0 4px; font-weight: 500;">Total Amount</p>
                                      <p style="color: ${color}; font-size: 22px; font-weight: 700; margin: 0;">Rs. ${fmt(order.totalAmount)}</p>
                                    </td>
                                  </tr>
                                </table>
                              </td>
                      ...
                      <tbody>
                        ${order.items
                          ...
                          <tr style="border-top: 1px solid #f1f5f9;">
                            <td style="padding: 12px 16px; color: #1e293b; font-size: 14px; font-weight: 500;">${item.name}</td>
                            <td style="padding: 12px 16px; text-align: center; color: #1e293b; font-size: 14px;">${item.quantity}</td>
                            <td style="padding: 12px 16px; text-align: right; color: #1e293b; font-size: 14px; font-weight: 500;">Rs. ${fmt(item.price * item.quantity)}</td>
                          </tr>
                        `
                          )
                          .join('')}
                        <tr style="background: #fafbfc; border-top: 2px solid #e2e8f0;">
                          <td colspan="2" style="padding: 14px 16px; text-align: right; color: #1e293b; font-size: 15px; font-weight: 600;">Total:</td>
                          <td style="padding: 14px 16px; text-align: right; color: ${color}; font-size: 18px; font-weight: 700;">Rs. ${fmt(order.totalAmount)}</td>
                        </tr>
                      </tbody>
                    </table>
                  </td>
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td style="text-align: center; padding: 8px 0 16px;">
                    <a href="${APP_URL}/orders/${order._id}" style="display: inline-block; background: ${color}; color: #ffffff; padding: 14px 48px; border-radius: 50px; text-decoration: none; font-size: 15px; font-weight: 600; box-shadow: 0 4px 14px ${softShadow};">
                      View Order Details →
                    </a>
                  </td>
                </tr>
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td style="text-align: center; padding-top: 8px;">
                    <p style="color: #94a3b8; font-size: 14px; margin: 0;">
                      Have questions? <a href="mailto:${SUPPORT_EMAIL}" style="color: #2B7A4B; text-decoration: none; font-weight: 600;">Contact Support</a>
                    </p>
                  </td>
                </tr>
              </table>
          ...
          <tr>
            <td style="background: #fafbfc; padding: 24px 30px; text-align: center; border-top: 1px solid #e2e8f0;">
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td>
                    <p style="color: #94a3b8; font-size: 12px; margin: 0 0 4px;">© ${YEAR} GreenScape. All rights reserved.</p>
                    <p style="color: #cbd5e1; font-size: 11px; margin: 0;">This email was sent regarding your order #${order.orderNumber}</p>
                    <p style="color: #cbd5e1; font-size: 11px; margin: 4px 0 0;">
                      <a href="${APP_URL}/unsubscribe" style="color: #94a3b8; text-decoration: none;">Unsubscribe</a>
                    </p>
                  </td>
                </tr>
              </table>
...
export const getOrderStatusEmailTemplate = (order, status) => {
  ...
  const getStatusMessage = (status) => {
    const messages = {
      pending:
        "Thank you for your order! We're preparing it for processing. You'll receive updates as your order progresses.",
      processing:
        "Your order is being carefully prepared by our team. We'll notify you when it ships.",
      shipped: 'Your order is on its way! Track your package for real-time delivery updates.',
      delivered:
        'Your order has been successfully delivered. We hope you love your new plants! 🌿',
      cancelled:
        'Your order has been cancelled as requested. A refund will be processed shortly.',
    };
    return messages[status] || 'Your order status has been updated.';
  };
                          ...
                          ${['pending', 'processing', 'shipped', 'delivered']
                            .map((s, index) => {
                              ...
                              const dotColor = isActive
                                ...
                                : '#e2e8f0';
                              return `
                              <span style="display: inline-block; width: 12px; height: 12px; border-radius: 50%; background: ${dotColor}; margin: 0 4px; ${
                                isCurrent ? 'box-shadow: 0 0 0 4px ' + softBorder + ';' : ''
                              }"></span>
                              ${
                                index < 3
                                  ? `<span style="display: inline-block; width: 40px; height: 2px; background: ${
                                      isActive ? '#2B7A4B' : '#e2e8f0'
                                    }; margin: 0 2px;"></span>`
                                  : ''
                              }
                            `;
                              ...
                              <td style="padding-top: 12px; border-top: 1px solid #e2e8f0;">
                                <table width="100%" cellpadding="0" cellspacing="0">
                                  <tr>
                                    <td style="width: 100%; text-align: center;">
                                      <p style="color: #94a3b8; font-size: 11px; margin: 0 0 4px; font-weight: 500;">Delivered On</p>
                                      <p style="color: #10B981; font-size: 14px; font-weight: 600; margin: 0;">${new Date(
                                        order.deliveredAt
