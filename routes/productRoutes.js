...
// backend/routes/product.route.js - PRODUCTION VERSION
import express from 'express';
import Product from '../models/Product.js';
import auth from '../middleware/adminAuth.js';
import cloudinary from '../config/cloudinary.js';
...
// 3. GET PRODUCT STATISTICS (before /:id)
router.get('/products/stats', async (req, res) => {
  try {
    const [totalProducts, activeProducts, featuredProducts, bestSellerProducts, outOfStock, lowStock] =
      await Promise.all([
        Product.countDocuments(),
        Product.countDocuments({ isActive: true }),
        Product.countDocuments({ isFeatured: true, isActive: true }),
        Product.countDocuments({ isBestSeller: true, isActive: true }),
        Product.countDocuments({ stock: 0 }),
        Product.countDocuments({ $expr: { $lte: ['$stock', '$lowStockAlert'] }, stock: { $gt: 0 } }),
      ]);
    ...
    console.error('❌ Error fetching product statistics:', err.message);
    res.status(500).json({ success: false, error: 'Server error' });
  }
});

// 4. CATEGORY STATS (before /:id)
router.get('/categories/stats', async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: { categories: categoryStats, subCategories: subCategoryStats },
    });
  } catch (err) {
...
// 7. CREATE PRODUCT (no multer — frontend sends image URL)
router.post('/products', createProduct);

// 8. UPDATE PRODUCT (no multer)
...
// 13. CLEANUP UNUSED IMAGES
router.delete('/cleanup-images', async (req, res) => {
  try {
    const products = await Product.find({}, 'image gallery thumbnail').lean();
    const usedImages = new Set();

    products.forEach((p) => {
      if (p.image) usedImages.add(p.image);
      if (p.thumbnail) usedImages.add(p.thumbnail);
      if (p.gallery) p.gallery.forEach((img) => usedImages.add(img));
    });
    ...
    console.error('❌ Error cleaning up images:', err.message);
    res.status(500).json({ success: false, error: 'Server error' });
  }
});
